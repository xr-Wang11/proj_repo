...
const entryFile = path.resolve(projectRoot, "web", "app-entry.js");
const outputFile = path.resolve(projectRoot, "web", "app.bundle.js");
const htmlTemplateFile = path.resolve(projectRoot, "web", "index.html");
const standaloneHtmlFile = path.resolve(projectRoot, "web", "index.standalone.html");
...
function wrapModuleSource(source) {
  return source.replace(/\\/gu, "\\\\").replace(/`/gu, "\\`").replace(/\$\{/gu, "\\${");
}

function buildBundleSource(modules, entryId) {
...
${module.source
  ...
  return `(function(modules, entryId) {
  var cache = {};
...
function buildBrowserBundle() {
  ...
  const bundleSource = buildBundleSource(modules, toProjectId(entryFile));
  fs.writeFileSync(outputFile, bundleSource, "utf8");
  return {
    bundleSource,
    entryId: toProjectId(entryFile),
    outputFile,
    moduleCount: modules.size,
  };
}

function buildStandaloneHtml(bundleResult = null) {
  const resolvedBundleResult = bundleResult || buildBrowserBundle();
  const htmlTemplate = fs.readFileSync(htmlTemplateFile, "utf8");
  const safeInlineBundleSource = resolvedBundleResult.bundleSource.replace(/<\/script/giu, "<\\/script");
  const standaloneSource = htmlTemplate.replace(
    /<script\s+src="\.\.\/app\.bundle\.js"><\/script>|<script\s+src="\.\/app\.bundle\.js"><\/script>/u,
    `<script>\n${safeInlineBundleSource}\n    </script>`
  );

  if (standaloneSource === htmlTemplate) {
    throw new Error("Cannot build standalone HTML: app.bundle.js script tag not found in web/index.html.");
  }
...
if (require.main === module) {
  ...
  const standaloneResult = buildStandaloneHtml(result);
  process.stdout.write(
    `Browser bundle built: ${path.relative(projectRoot, result.outputFile)} (${result.moduleCount} modules)\n`
  );
  process.stdout.write(
    `Standalone HTML built: ${path.relative(projectRoot, standaloneResult.outputFile)}\n`
  );
}
